# taken from: https://github.com/microsoft/renee/blob/main/utils/CreateTokenizedFiles.py
"""
...
--data-dir artifacts/slm-generations/LF-ORCAS-800K \
--max-length 32 \
--tokenizer-type bert-base-uncased \
--num-threads 32

Please the file below modify accordingly for trn_doc, tst_doc, lbl or rewrite_doc !!!
...
rewrite = [x.strip() for x in open(f'{DATA_DIR}/raw_rewrites.txt', "r", encoding="utf-8").readlines()]
...
#     Y = [x.strip() for x in open(f'{DATA_DIR}/raw/Y.txt', "r", encoding="utf-8").readlines()]
#     print(len(Y))
#     print(Y[:10])
...
print("Dumping for rewrite...")
tokenize_dump_memmap(rewrite, tokenization_dir, tokenizer, max_len, "rewrite_doc", args.num_threads)

# if args.tokenize_label_texts:
#     print("Dumping for Y...")
#     tokenize_dump_memmap(Y, tokenization_dir, tokenizer, max_len, "rewrite_doc", args.num_threads)
